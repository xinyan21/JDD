 ...
 * @since 1.0
 */
public class QueryRoseListRequest implements CallBack.SchedulerCallBack {
    public static final String SORT_FLAG_UP = "up";
    public static final String SORT_FLAG_DOWN = "down";
    private MemberCache mCache = DataCache.getInstance().getCache();
    private Parameter mParameter;
    private ResponseAction mAction;
    private int mTaskId;

    public QueryRoseListRequest(Parameter parameter, ResponseAction action) {
    ...
    public void handler(MessageAction messageAction) {
        String url = "";
        if (mParameter.isExistKey("sortFlag") && SORT_FLAG_UP.equals(mParameter.getString("sortFlag"))) {
            url = ConfigStore.getConfigValue("system", "PRICE_SERVER_URL")
                    + ConfigStore.getConfigValue("system", "URL_QUERY_RISE");
        } else if (SORT_FLAG_DOWN.equals(mParameter.getString("sortFlag"))) {
            url = ConfigStore.getConfigValue("system", "PRICE_SERVER_URL")
                    + ConfigStore.getConfigValue("system", "URL_QUERY_DROP");
        }
        ...
        byte[] bytes = request.get(url, null);
        if (null != bytes) {
            ...
            try {
                ...
                if (1 == jsonResult.optInt("code")) {
                    ...
                    mCache.addCacheItem(CacheKey.KEY_STOCK_ROSE_DROP_LIST + mTaskId, data);
                    bundle.putString(
                            String.valueOf(mTaskId), CacheKey.KEY_STOCK_ROSE_DROP_LIST + mTaskId
                    );
                    messageAction.transferAction(ResponseAction.RESULT_OK, bundle, mAction);
                }
